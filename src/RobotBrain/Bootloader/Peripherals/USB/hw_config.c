...
extern volatile uint8_t USB_SendBuffer[VIRTUAL_COM_PORT_DATA_SIZE] ;
extern volatile uint32_t USB_PacketReceived;
extern volatile uint8_t USB_ReceivedLength;

uint8_t USB_ReceiveBuffer[64];
...
void Enter_LowPowerMode(void)
{
  ...
  bDeviceState = SUSPENDED;
	...
	//Reset System Clock Speed 
	
	
	if(api.USB.OnUSBDisconnect != NULL)
		api.USB.OnUSBDisconnect();
...
void Leave_LowPowerMode(void)
{
  ...
  SystemInit();
	...
	//Set System Clock to 72MHz so that it can talk to USB Protocol
	SYSCLK_72Mhz();

	
	/*On USB Connect*/
	if(api.USB.OnUSBConnect != NULL)
		api.USB.OnUSBConnect();
